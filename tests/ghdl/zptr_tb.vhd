-- zptr_tb.vhd — testbench for zptr_test.S
--
...
architecture sim of zptr_tb is
...
begin
    ...
    begin
        ...
        assert gpio_ddr = to_unsigned(255, 8)
            report "FAIL zptr_tb: gpio_ddr = " & integer'image(to_integer(gpio_ddr))
                   & ", expected 255"
            severity error;
        assert gpio_port = to_unsigned(195, 8)
            report "FAIL zptr_tb: gpio_port = " & integer'image(to_integer(gpio_port))
                   & ", expected 195 (0xC3)"
            severity error;
