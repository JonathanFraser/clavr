-- cov_arith_tb.vhd — testbench for cov_arith.S (carry arithmetic, compares, skips, 16-bit ops) (control-flow instruction coverage)
--
...
entity cov_arith_tb is
end entity cov_arith_tb;

architecture sim of cov_arith_tb is
...
begin
    ...
    begin
        ...
        assert gpio_ddr = to_unsigned(255, 8)
            report "FAIL cov_arith_tb: gpio_ddr = " & integer'image(to_integer(gpio_ddr))
                   & ", expected 255"
            severity error;
        assert gpio_port = to_unsigned(165, 8)
            report "FAIL cov_arith_tb: gpio_port = " & integer'image(to_integer(gpio_port))
                   & ", expected 165 (0xA5)"
            severity error;
