-- gpralias_tb.vhd — testbench for gpralias_test.S
--
...
entity gpralias_tb is
end entity gpralias_tb;

architecture sim of gpralias_tb is
...
begin
    ...
    begin
        ...
        assert gpio_ddr = to_unsigned(255, 8)
            report "FAIL gpralias_tb: gpio_ddr = " & integer'image(to_integer(gpio_ddr))
                   & ", expected 255"
            severity error;
        assert gpio_port = to_unsigned(153, 8)
            report "FAIL gpralias_tb: gpio_port = " & integer'image(to_integer(gpio_port))
                   & ", expected 153 (0x99)"
            severity error;
